...
pwd <- "C:/Users/Omar/Desktop/HPC/Prace_Sohpc2021_Shape/"
...
ExecuteCode <- function(varConsumerID){
  ...
  for (i in c(1:length(varConsumerIDs))){
    ...
    varConsumerID = varConsumerIDs[i]
    ...
    write.csv(x=unlist(listLoadFile), file=paste("./TimeResults/Parallel/Lapply/", varConsumerID,"_TimeLoadFile.csv"))
    write.csv(x=unlist(listCreateSaveModel), file=paste("./TimeResults/Parallel/Lapply/", varConsumerID,"_TimeCreateSaveModel.csv"))
    write.csv(x=unlist(listGeneratePrediction), file=paste("./TimeResults/Parallel/Lapply/", varConsumerID,"_TimeGenerationPrediction.csv"))
  }
